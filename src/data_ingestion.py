...
class DataIngestion:
    def __init__(self, config):
        ...
        logger.info(f"Data ingestion start with {self.bucket_name} and file is {self.bucket_filename}")

    def download_csv_from_gcp(self):
        try:
            client = storage.Client()
            bucket = client.bucket(self.bucket_name)
            blob = bucket.blob(self.bucket_filename)
            ...
            raise CustomException("Faile to download csv file " , e)
    ...
    def split_data(self):
        try:
            ...
            raise CustomException("Faile to split data " , e)
