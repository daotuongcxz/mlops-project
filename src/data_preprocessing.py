...
class DataProcessor:
    ...
    def preprocess_data(self, df):
        try:
            ...
            logger.info("Dropping the colums")
            df.drop(columns=['Unnamed: 0', 'Booking_ID'] , inplace=True)
            df.drop_duplicates(inplace=True)

            cat_cols = self.config["data_processing"]["categorical_colums"]
    ...
    def select_features(self, df):
        try:
            ...
            num_features_to_select = self.config["data_processing"]["number_of_features"]
            ...
            logger.info("Feature selection completed successed")
    ...
    def save_data(self, df, file_path):
        try:
            logger.info("Saving out data in processed folder")
            ...
            raise CustomException("Error while save data", e)
